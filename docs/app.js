const CAPTURE_PARAMS=new URLSearchParams(location.search),CAPTURE_MODE=CAPTURE_PARAMS.get('capture')==='1',CAPTURE_CATEGORY=CAPTURE_PARAMS.get('category'),DESKTOP_LAYOUT=window.matchMedia('(min-width: 900px)').matches;if(CAPTURE_MODE)document.body.classList.add('capture-mode');
...
const semanticCellTone=(cell,isHeader,ci)=>{if(isHeader||ci<4)return null;const number=numericCellValue(cell.v);if(number===null||number===0)return{backgroundColor:'#ffffff',color:'#111111',className:''};return number>0?{backgroundColor:'rgb(183 223 207)',color:'#111111',className:'positive-data-cell'}:{backgroundColor:'rgb(241 198 198)',color:'#111111',className:'negative-data-cell'}};
...
Promise.all([fetch('sheet-data.json').then(r=>r.json()),fetch('sheet-layout.json').then(r=>r.json())]).then(([sheet,layout])=>{
  ...
  let selectedPeriod='1d',selectedCategory='all',columnLayout='period',labelType='name',zoomLevel=DESKTOP_LAYOUT?100:150,viewMode=DESKTOP_LAYOUT?'desktop':'mobile',compactHeaders=true,activeMenuTrigger=null,activePanel=null,canReturnToSettings=false;
  ...
  const headerHeightFor=()=>DESKTOP_LAYOUT?(compactHeaders?34:54):(compactHeaders?18:40);
  const rowHeightFor=(sourceRowIndex,isHeader,isTotal)=>isHeader?headerHeightFor():isTotal?(DESKTOP_LAYOUT?(compactHeaders?28:34):(compactHeaders?16:20)):images[String(sourceRowIndex)]?(DESKTOP_LAYOUT?32:20):(DESKTOP_LAYOUT?30:Math.min(rows[sourceRowIndex]||18,18));
  ...
  const openMenu=(panel,opener,fromSettings=false)=>{
    ...
    mount.inert=true;backdrop.querySelector('.period-menu-body').scrollTop=0;
    requestAnimationFrame(()=>{if(!backdrop.hidden)backdrop.classList.add('open')});if(opener)opener.setAttribute('aria-expanded','true');(backButton.hidden?closeButton:backButton).focus();
  };
  const closeMenu=({restoreFocus=true}={})=>{
    ...
    if(opener){opener.setAttribute('aria-expanded','false');if(wasOpen&&restoreFocus)opener.focus()}
  };
  const render=()=>{
  ...
  document.getElementById('header-home').addEventListener('click',event=>{closeMenu({restoreFocus:false});mount.scrollTo({left:0,top:0,behavior:'auto'});event.currentTarget.focus()});
